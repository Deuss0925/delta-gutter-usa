...
export function Warranty() {
  return (
    <Section id="warranty">
      <SectionHeading
        ...
        title={
          <>
            ...
            <span className="text-blue-400">labor and materials.</span>
          </>
        }
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {cards.map((card, i) => (
          <Reveal key={card.title} delay={i * 0.1}>
            <div className="h-full rounded-3xl border border-ice/10 bg-navy-800/40 p-8 transition-colors hover:border-blue-400/30">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-4 text-2xl text-ice">{card.title}</h3>
                </div>
                <div className="text-right leading-none">
                  <span className="font-display text-6xl font-extrabold text-blue-400 sm:text-7xl">
                    <CountUp to={card.years} />
                  </span>
                  <span className="mt-1 block font-mono text-xs uppercase tracking-[0.2em] text-ice/60">
                    Years
                  </span>
                </div>
              </div>
              ...
              <ul className="mt-6 grid gap-2.5 sm:grid-cols-2">
                {card.covers.map((item) => (
                  <li
                    key={item}
                    className="flex items-start gap-2 text-[15px] text-ice/75"
                  >
                    <Check
      ...
      <Reveal
        delay={0.15}
        className="mt-8 flex flex-col items-center justify-between gap-5 rounded-2xl border border-blue-400/20 bg-blue-500/[0.06] p-6 sm:flex-row"
      >
        <p className="flex items-center gap-3 text-center text-ice sm:text-left">
          <Clock className="size-5 shrink-0 text-blue-400" aria-hidden />
